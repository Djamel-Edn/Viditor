...
const Plan = ({icon:MainIcon,price,name,slogan,option1,option2,option3,option4,buttonIcon:ButtonIcon,type,buttonLink}:PlanProps) => {
  return (
    <div className='relative flex flex-col itens-center gap-2 '>
        <div>
            <Icon icon={MainIcon} size={type==="limited" ? 'lg':'sm'}/>
        </div>
        <p className={`w-fit text-center text-xs font-semibold uppercase rounded-full border-[1.5px] px-3 py-1 ${type==="limited" ? 'text-slime-400 border-line-400 mt-8':'text-cyan-500 border-cyan-500 mt-4'}`}>{name}</p>
